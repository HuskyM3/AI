    ...
    * Otherwise, if all literals of a clause are False, then the whole clause
    ...
    while True:
        ...
        for clause in clauses:
            ...
            is_true = False
            for unit, val in clause:
                assignedVal = valuation.get(unit, None)
                
                if assignedVal is not None and assignedVal == val:
                    is_true = True
                    break
            if is_true:
                continue
            ...
            for unit, val in clause:
                ...
                if unit not in valuation:
                    unassigned.append((unit, val))

            if len(unassigned) > 1:
                continue

            elif len(unassigned) == 1:
                literal, val = unassigned[0]
                valuation[literal] = val
                found = True
                break
...
def dpll(valuation, clauses, variables):
    ...
    (satisfiability, resultVal) = dpll(valuation, clauses, variables)
    if satisfiability:
